...
void CSVWriter::printCSVDouble_V2(int rows, int cols, std::vector<double> network){
	...
	for (int r = 0; r < rows; r++)
	{
		for (int c = 0; c < cols; c++)
		{
			...
			// ofs << (int)network[c+r*cols]  << this->delimeter << (int)network[c+r*cols + 1]  << this->delimeter << (int)network[c+r*cols + 2]  << this->delimeter << network[c+r*cols + 3] << this->delimeter << network[c+r*cols + 4] << "\n";
			ofs << (int)network[c+r*cols]  << this->delimeter << (int)network[c+r*cols + 1]  << this->delimeter << (int)network[c+r*cols + 2]  << this->delimeter << network[c+r*cols + 3] << this->delimeter << network[c+r*cols + 4] << this->delimeter << network[c+r*cols + 5] << this->delimeter << network[c+r*cols + 6] << "\n";
			c+=cols;
		}
