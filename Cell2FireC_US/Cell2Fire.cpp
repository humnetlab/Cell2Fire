...
void Cell2Fire::Results(){
    ...
    ******************************************************************************/
    ...
    if(this->args.OutMessages){
        ...
        CSVPloter.printCSVDouble_V2(this->FSCell.size() - this->nIgnitions, 7, this->FSCell); 
...
int main(int argc, char * argv[]){
    ...
    for(ep = 1; ep <= args.TotalSims * 10; ep++){
        ...
        for (tstep = 0; tstep <= Forest.args.MaxFirePeriods * Forest.args.TotalYears ; tstep++){   
            ...
            // Get the current time before the step
            auto step_start = std::chrono::high_resolution_clock::now();

            Forest.Step(generator);
            // DEBUGprintf("\nDone: %d", Forest.done);

            // Get the current time after the step
            ...
            // Print the duration of the step
            std::cout << "Time taken for step " << tstep << ": " << duration.count() << " microseconds" << std::endl;
        ...
        // Get the current time after the loop ends
        ...
        std::cout << "Total time taken per sim: " << total_duration.count() << " microseconds" << std::endl;
    }
