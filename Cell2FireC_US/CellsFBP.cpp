...
/************************************************************************************************************ 
    ...
    # 6. Anderson (1983) - heavy slash       # 7. Alexander et al. (1985)   
    # 8. KITRAL System  
************************************************************************************************************/
...
/********************************************************************************************/
std::vector<int> CellsFBP::manageFire(int period, 
                                      std::unordered_set<int> & AvailSet,      
                                      inputs df_ptr[], 
                                      fuel_coefs * coef, 
                                      std::unordered_map<double, std::vector<double>> *kdict_ptr,
    ...
    if (HROS > args->ROSThreshold) {
        ...
        ros_boosters[315] = 1;
        ros_boosters[90] = 1.0;
        ros_boosters[270] = 1.0;
        ros_boosters[180] = 1.0;
        ros_boosters[135] = 1;
        ros_boosters[225] = 1;
        ...
        for (auto&  _angle : this->ROSAngleDir) {
            ...
            if (this->fireProgress[nb] >= this->distToCenter[nb]) {
                ...
                FSCell->push_back(this->fireProgress[nb]);
                FSCell->push_back(this->distToCenter[nb]);
...
//     if (cartesianAngle < 0) cartesianAngle += 360; 
    ...
    if (HROS > args->ROSThreshold) {
        ...
        ros_boosters[0] = 1.0;
        ros_boosters[45] = 1.0;
        ros_boosters[315] = 1.0;
        ros_boosters[90] = 1.0;
        ros_boosters[270] = 1.0;
        ros_boosters[180] = 1.0;
        ros_boosters[135] = 1.0;
        ros_boosters[225] = 1.0;
