...
__maintainer__ = "Cristobal Pais, Minho Kim"
*/
...
weatherDF wdf[270];
inputs df [17800000];
std::unordered_map<int, std::vector<float>> BBOFactors; //static 
...
// std::unordered_map<double, std::vector<double>> * kdict_ptr; // Initialize
...
/******************************************************************************
                                    Constructor 
*******************************************************************************/
Cell2Fire::Cell2Fire(arguments _args) : CSVWeather(_args.InFolder + "Weather.csv", ","), CSVForest(_args.InFolder + "Forest.asc", " "), CSVkopt("kopt.csv", ",")
{
    // Aux
    int i;
    ...
    ********************************************************************/
    ...
    /* K correction factors */ // TODO: MHK (022223)
    // If args.kOptsTable exists, then create a flag and define K1 - K5
    // If kOptsTable is false, then we need default values by creating an empty table with the correct type
    // Alternatively, just add to pointer (this->args_ptr)
    // std::cout << "KOPTION : " << this->args.KOption << std::endl;

    if(strcmp(this->args.KOption.c_str(), EM) != 0){
        // // koptDF koptdf;
        // this->koptDF = this->CSVkopt.getData();
        
        // // Create empty df
        // kopt_ptr = & koptdf[0];
        
        // // Populate
        // this->CSVkopt.parseKopt(kopt_ptr, this->koptDF);
        
        // koptDict kdict;
        // kdict_ptr = & kdict[0];
        // std::cout << "Debugging in Cell2Fire : Pointer" << std::endl;

        // MHK Must declare a new dereferenced pointer here for the next lines to work!!
        // std::unordered_map<double, std::vector<double>> *kdict_ptr = new std::unordered_map<double, std::vector<double>>;

        CSVReader CSVKopt("", ""); // define object
        CSVKopt.parseKoptDict("k_opt.csv",kdict_ptr);

        // Check pointer
        if (!kdict_ptr) {
            std::cerr << "Cell2fire Error: The input pointer !kdict_ptr is null." << std::endl;
        // return;
        }

        if (this->args.verbose){
            std::cout << "Debugging in Cell2Fire : Check to see contents of kdict_ptr" << std::endl;
        ...
        // koptDict = *dict_ptr // assign value of pointer to koptDict
...
bool Cell2Fire::RunIgnition(std::default_random_engine generator){
    ...
    *******************************************************************/
    ...
    if (!this->noIgnition) {
        ...
        // Print sets information // MHK: 042022 - commented out due to large size
        // if (this->args.verbose){
        ...
        */
        if (it->second.ROSAngleDir.size() > 0) {
            if (!this->args.BBOTuning){  //&df[cell-1] replaced by full df for getting the slopes
                ...
                // Check pointer
                if (kdict_ptr == nullptr) {
                    std::cout << "Cell2fire Error: kdict_ptr is null\n";
                }
                ...
                // TODO: Pass the slope factor or the slopes of the adjacent cells
...
void Cell2Fire::GetMessages(std::unordered_map<int, std::vector<int>> sendMessageList){
    ...
    // Information of the current step // MHK: 042022 - commented out due to large size
    if (this->args.verbose){
    ...
    *************************************/
    // Mesages and no repeat 
     if (this->messagesSent && !this->repeatFire) {
        ...
        // Display info for debugging // MHK: 042022 - commented out due to large size
        // if(this->args.verbose){
        // 	printSets(this->availCells, this->nonBurnableCells, this->burningCells, this->burntCells, this->harvestCells);
        // }
...
void Cell2Fire::Results(){
    ...
    ******************************************************************************/
    ...
    if(this->args.OutMessages){
        ...
        CSVPloter.printCSVDouble_V2(this->FSCell.size() - this->nIgnitions, 5, this->FSCell); 
        // Change number of features in the third input (MHK)
    }
}
...
void Cell2Fire::Step(std::default_random_engine generator){
    ...
    if (this->args.verbose){
        ...
        // printSets(this->availCells, this->nonBurnableCells, this->burningCells, this->burntCells, this->harvestCells); // MHK: 042022 - commented out due to large size
        std::cout << "********************************************" << std::endl;
    }
...
int main(int argc, char * argv[]){
    ...
    // CP: Modified to account the case when no ignition occurs and no grids are generated 
    // (currently we are not generating grid when no ignition happened, TODO)
    for(ep = 1; ep <= args.TotalSims * 10; ep++){
        ...
        // Enforces to satisfy the total number of simulations (if no ignition, find another cell
        // until we obtain the TotalSim asked)
