...
parseArgs(int argc, char* argv[], arguments* args_ptr)
{
    ...
    if (simulator_option)
    {
        ...
        // std::cout << "DEBUGGING SIMULATOR: " << s << std::endl;
    ...
    //--MCNumber (v2)
    char * mcnumber = getCmdOption(argv, argv + argc, "--mcnumber");
    if (mcnumber){
        printf("MC Number: %s \n", mcnumber);
        args_ptr->mcnumber = std::stoi (mcnumber ,&sz); 
    } 
    else args_ptr->mcnumber = dmcnumber;
    ...
    // Elliptical factors to avoid long-shaped fires due to CA distortion (v2)
    //--kopt (v2)
